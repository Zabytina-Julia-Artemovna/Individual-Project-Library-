...
public:
    Book() :
        _book_id(0),
        _count_of_books(0),
        _year_of_publication(0),
        _current_state(free),
        _author("Unknown"),
        _language("Unknown"),
        _publishing_house("Unknown"),
        _book_name("Unknown") {}
    Book(size_t book_id, size_t count_of_books, size_t year_of_publication,
        const std::string& author, const std::string& language,
        const std::string& publishing_house, const std::string& book_name) :
        _book_id(book_id),
        _count_of_books(count_of_books),
    ...
    void set_count_of_books(size_t count_of_books) {
        _count_of_books = count_of_books;
    }
